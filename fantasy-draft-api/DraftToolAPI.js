...
import {
    ...
    deletePlayer,
    resetPlayers,
} from './Services/DraftToolService.js'
...
async function addPlayerEndpoint(request, response) {
    try {
        ...
        return response.status(500).send({ error: 'Error adding player to DB' })
    }
}
...
async function fetchPlayersEndpoint(request, response) {
    try {
        ...
        return response.status(200).json(players) //returns an array instead of a nested object
    } catch (error) {
        return response
            .status(500)
...
async function fetchTeamEndpoint(request, response) {
    try {
        ...
        return response
            .status(500)
            .json({ error: 'Error getting players from DB' })
    }
}
