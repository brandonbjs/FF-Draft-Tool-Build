        ...
        return (
            <div>
                <div className="banner">
                    <img
                        src="/FFDT-gh-pages/images/transparent_ffdt_banner.png"
                        alt="Fantasy Football Draft Tool"
                        style={{
                            width: '13em',
                            height: 'auto',
                            position: 'absolute',
                ...
                <div className="buttonWrapper">
                    {!draftStarted && !isEndOfDraft && (
                        <button
                            className="startDraftButton"
                            onClick={this.props.handleStartDraft}
                        >
                            Start Draft
                        </button>
                    )}
                ...
                {draftStarted && !isEndOfDraft && (
                    <div className="countdownDiv">
                        Round {roundNum} - Team {pickingId} is drafting:{' '}
                        {countdown} seconds
                    </div>
                )}
                {isEndOfDraft && (
                        ...
                        teams.map((team) => (
                            ...
                            >
                                <h4
                                    className="teamName"
                                    onClick={() =>
                                        this.handleTeamNameClick(team.id)
                                    }
                                >
                                    {team.name}
                                </h4>
                                <ul className="playerList">
                                    <li>QB: {team.players.QB?.name}</li>
                                    <li>RB1: {team.players.RB1?.name}</li>
                                    <li>RB2: {team.players.RB2?.name}</li>
                                    <li>WR1: {team.players.WR1?.name}</li>
                                    <li>WR2: {team.players.WR2?.name}</li>
                                    <li>TE: {team.players.TE?.name}</li>
                                    <li>FLEX1: {team.players.FLEX1?.name}</li>
                                    <li>FLEX2: {team.players.FLEX2?.name}</li>
                                    <li>DST: {team.players.DST?.name}</li>
                                    <li>K: {team.players.K?.name}</li>
                                </ul>
                            </div>
                        ))
                    }
